  ...
  return (
    <DropzoneContext.Provider value={{ ...restProps }}>
      <div
        {...getRootProps({
          className: cn(
            'text-foreground border-border rounded-[var(--radius-panel)] border p-6 text-center transition-colors motion-reduce:transition-none',
            className,
            isSuccess ? 'border-solid' : 'border-dashed',
            isActive && 'border-border-brand bg-brand-200',
            isInvalid && 'border-border-destructive bg-destructive-200',
          ),
        })}
      >
        <input {...getInputProps()} />
