# Recall: the visual system

Recall uses the Supabase design system. Not a system inspired by it, not a reinterpretation of it. The actual tokens, vendored from `supabase/supabase` and refreshed by `scripts/sync-tokens.mjs`. A Supabase designer should open this app and find the system they already maintain.
...
Overrides local to Recall live in `web/styles/tokens.css`, which loads last and is documented in `docs/design.md` with a justification per token.
