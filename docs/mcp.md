...
Supabase is shipping BYO MCP at Select, and Recall should use that feature rather
than hand-roll around it. Building a bespoke MCP server now means throwing it
away in three weeks and losing the better demo, which is Recall's MCP server
generated by the product we are launching.

So `supabase/functions/mcp-server/` exists with the route registered, auth wired
to the existing flow, and one tool. The four real tools are specified here and
implemented later.
...
Four tools, matching the `TODO` in the stub. Every one of them reaches the
database through the caller's own JWT with row level security enabled, so the
...
text Recall actually indexed rather than the original file bytes.
...
The document is written immediately and the chunking and embedding run as an
ingest job, so the tool returns before the note is searchable. The caller polls
...
The Supabase Library blocks `mcp-server` and `oauth-consent-nextjs` are the
