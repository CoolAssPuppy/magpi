...
Supabase is shipping BYO MCP at Select, and Magpi should use that feature rather
than hand-roll around it. Building a bespoke MCP server now means throwing it
away in three weeks and losing the better demo, which is Magpi's MCP server
