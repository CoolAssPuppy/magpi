...
| ---- | -------------- | ------------------------------------------------------------------------------------------------------------- | -------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F001 | Security       | `supabase/schemas/90_policies.sql:52`, `95_grants.sql:25`                                                     | Critical | M   | **Verified.** A space member can `update spaces set org_id = <other org>` and can flip a team space to `kind = 'org'`. Nothing constrains `documents.org_id`, `chunks.org_id`, `connections.org_id`, `ingest_jobs.org_id` to match.         | Narrow the grant to `(name, dreaming_enabled)`; add a `with check` pinning `org_id` and `kind`; add `spaces (id, org_id)` unique and composite keys from the four content tables. |
| F002 | Security       | `web/app/(app)/documents/actions.ts:11,49`                                                                    | Critical | S   | **Verified.** `storagePath` is `z.string().min(1)`, written by the service client; `jobs/ingest.ts:111` reads that path with no space check. A caller indexes another space's object into their own.                                        | Constrain to `^${spaceId}/`, or derive the path server-side. Constrain `mimeType` to the allowed set.                                                                             |
| F003 | Correctness    | `supabase/functions/_shared/jobs/ingest.ts:296`                                                               | Critical | S   | A transient `SourceError` writes terminal `failed`, so the `attempts < 3` budget is unreachable and the driver message promising a retry is false.                                                                                          | Write `queued` for `needsReconnect === false` and let the cap retire it.                                                                                                          |
| F004 | Correctness    | `supabase/functions/ingest-worker/index.ts:37`, `schemas/80_functions.sql:318`                                | Critical | M   | The JS `retireAbandoned` sweep and the SQL reclaim CTE cover the same rows with opposite outcomes; the sweep runs first, so the CTE never matches.                                                                                          | Delete the JS sweep, let the CTE requeue and the attempt cap produce the terminal row.                                                                                            |
| F005 | Correctness    | `supabase/functions/_shared/sources/notion.ts:190`                                                            | Critical | M   | First pass over a workspace larger than `MAX_REQUESTS` pages advances the cursor to the newest page while search sorts descending, so the backlog is never read.                                                                            | Return the last `next_cursor` when `hasMore`; make the cursor a discriminated value.                                                                                              |
| F006 | Correctness    | `supabase/functions/_shared/sources/linear.ts:211`                                                            | High     | M   | Cursor advances to newest `updatedAt` from one page of 50; `orderBy: updatedAt` has no direction, and only our own fixture says ascending.                                                                                                  | Paginate with `after: endCursor`, or pin the direction and add a two-pass test.                                                                                                   |
| F007 | Correctness    | `supabase/functions/connections-claim/index.ts:90`                                                            | High     | S   | `.eq('external_account_id', null)` never matches, so a provider returning no account label creates a fresh connection with a live token on every reconnect.                                                                                 | Use `.is()` when null; add the partial unique index.                                                                                                                              |
| F008 | Schema drift   | `supabase/schemas/80_functions.sql:13` and 10 others                                                          | High     | S   | Eleven of eighteen functions are revoked only in `migrations/20260909161000`, so a shadow database built from `schemas/` alone leaves them executable by PUBLIC. Not current exposure; the next regeneration is.                            | Copy the eleven revokes into `80_functions.sql` beside each grant.                                                                                                                |
| F009 | Correctness    | `web/lib/analytics/queries.ts:281`                                                                            | High     | S   | `usage_events` with `kind = 'query'` is read and written nowhere, so the meter reads zero and `plan_monthly_query_limit` is enforced by nothing.                                                                                            | Insert on a successful chat turn; add a `check_query_allowed` gate.                                                                                                               |
| F010 | Correctness    | `web/lib/analytics/queries.ts:194`                                                                            | High     | M   | `documents.last_retrieved_at` and `retrieval_count` are written by nothing, so the dead-content panel reports every document as dead.                                                                                                       | Update them in the search path, or drop the panel until they are written.                                                                                                         |
| F011 | Error handling | `web/app/(app)/chat/actions.ts:40,57,74`, `spaces/actions.ts:30,38,56,75`, `documents/actions.ts:44,62,71,90` | High     | M   | Eleven paths return the raw `PostgrestError` string, so a policy denial renders as `new row violates row-level security policy for table "conversations"`. Four other action files map to product copy, so there are two conventions.       | Map to product copy the way `admin/members/actions.ts:51` does; log the raw error server-side.                                                                                    |
| F012 | Correctness    | `web/lib/chat/answer.ts:103`                                                                                  | High     | S   | `setCondensedQuery` and `setTitle` run inside the try that yields the error event, after `done`. A failed title write deletes a delivered answer from the screen.                                                                           | Own try around the post-`done` block, catching and yielding nothing.                                                                                                              |
...
| F016 | Architecture   | `supabase/functions/_shared/sources/contract.ts:41`                                                           | High     | M   | `ChangePage.hasMore` is computed by four drivers, asserted in eight tests, and read by no caller, so a partial sync never schedules another pass.                                                                                           | Loop in `runSyncJob` until spent, or return it in `SyncResult` and re-queue.                                                                                                      |
| F017 | Performance    | `web/app/(app)/admin/members/page.tsx:58`                                                                     | High     | M   | One `auth.admin.getUserById()` per member inside `Promise.all`, over an unlimited query. The spec's stated scale is 4,000 people.                                                                                                           | `listUsers({ page, perPage })` and join in memory, or a security-definer function. Paginate.                                                                                      |
| F018 | Docs           | `docs/retrieval.md:100`                                                                                       | High     | S   | Says "Status: implemented" and lists a 100-token merge, a 1,200-token ceiling, a heading rule and real tokenization. None exists; `chunking.ts:38` is `Math.ceil(text.length / 4)` and its own comment says "not a real tokenizer".         | Rewrite the table from the code, or implement the four rules.                                                                                                                     |
| F019 | Docs           | `docs/mobile-spec.md:46`                                                                                      | High     | S   | Names `web/lib/strings/` as the string catalog and every entry cites keys from it. The module does not exist and `mobile-spec-check.mjs` only checks routes.                                                                                | Build it or mark the field planned.                                                                                                                                               |
| F020 | Correctness    | `supabase/functions/dream-run/index.ts:55`                                                                    | High     | S   | Inserts a run as `queued` then runs it inline without claiming, so a cron tick can execute the same run twice. `dream-worker` claims for exactly this reason.                                                                               | Insert as `running`, or claim before running.                                                                                                                                     |
...
| F022 | Performance    | `web/lib/supabase/context.ts:22`                                                                              | Medium   | S   | `getSessionContext` is not wrapped in `cache()`; an admin page load makes three `auth.getUser()` round trips plus three `org_members` selects.                                                                                              | Wrap it and `resolveAdminAccess` in React `cache()`.                                                                                                                              |
...
| F027 | Types          | `web/lib/spaces/spaces.ts:66`, `documents.ts:89`                                                              | Medium   | S   | Two assertions bridging a query into a hand-written row type that shadows the generated one. `analytics/queries.ts:105` reads the same shape with no cast, proving the generated types handle it.                                           | Delete both assertions and both hand-written types.                                                                                                                               |
| F028 | Types          | `web/app/auth/confirm/route.ts:10`                                                                            | Medium   | S   | `searchParams.get('type') as EmailOtpType` asserts an untrusted query parameter and hands it to `verifyOtp`. The one place in the app that asserts instead of parsing.                                                                      | `z.enum([...]).safeParse`.                                                                                                                                                        |
| F029 | Correctness    | `web/lib/documents/documents.ts:79`                                                                           | Medium   | S   | The embedded `ingest_jobs` has no order or limit, so `row.ingest_jobs[0]` is arbitrary. The detail page orders correctly, so list and detail can disagree about whether a document imported.                                                | Order and limit the embed.                                                                                                                                                        |
| F030 | Performance    | `web/lib/chat/store.ts:113`                                                                                   | Medium   | S   | `loadMessages` selects every message in a conversation with no limit, on every POST, and only eight turns are used. The chunks query is likewise unbounded.                                                                                 | Limit for the request path; paginate chunks.                                                                                                                                      |
| F031 | Design         | `web/styles/tokens.css:19`                                                                                    | Medium   | S   | The named z-index scale `DESIGN.md` mandates has zero consumers; five overlay primitives use raw `z-50`, colliding with `--z-toast`.                                                                                                        | Apply the tokens, or delete the scale from tokens and docs.                                                                                                                       |
| F032 | Design         | `web/components/ui/dialog.tsx:41` and 5 others                                                                | Medium   | M   | Six elements pair `border` with `shadow-lg`, which `DESIGN.md` bans by name, and use `rounded-md` where `--radius-panel` exists. These are the shadcn defaults the spec says to rewrite before entry.                                       | Pick one per element, switch the radii, add a check.                                                                                                                              |
| F033 | Design         | `web/components/ui/*` (30+ sites)                                                                             | Medium   | M   | The primitives are written against shadcn compat aliases that resolve only through `compat.css`, which upstream has marked for deletion. The next token sync silently unstyles twelve files.                                                | See open questions.                                                                                                                                                               |
| F034 | Consistency    | `web/app/(app)/documents/page.tsx:12`, `spaces/page.tsx:11`                                                   | Medium   | S   | Two ways to get a server client in a page: four resolve `getSessionContext()`, two call `createClient()` directly and have no `orgId`.                                                                                                      | Standardize on `getSessionContext()`.                                                                                                                                             |
...
| F036 | Error handling | `web/app/api/chat/route.ts:105`                                                                               | Medium   | S   | `consumeRateLimit` throws and nothing catches it, so the request 500s, contradicting the comment three lines above promising a typed JSON body.                                                                                             | Return `failure('server_error', ...)`.                                                                                                                                            |
